...
void MainWindow::removeWorkspace()
{
    ...
    reply = QMessageBox::question(this, tr("Delete Workspace"),
                                 tr("Are you sure you want to delete workspace \"%1\"?").arg(workspaceName),
                                 QMessageBox::Yes|QMessageBox::No);
    if (reply == QMessageBox::Yes) {
        QString sql = "DELETE FROM Workspaces WHERE id = " + QString::number(workspaces[workspaceName]->getId()) + ";";
        executeSQL(sql);
        ...
        showWorkspaces();
        if (currentWorkspaceLabel->text().contains(workspaceName)) {
